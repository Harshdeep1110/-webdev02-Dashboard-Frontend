...
function renderBoard() {
    ...
    a && (a.innerHTML = "", AppState.columns.forEach(function(b) {
        var c = createColumnElement(b);
        a.appendChild(c)
    }), a.appendChild(document.createElement("div")), updateActivityBadge())
}
...
function renderActivityLog() {
    ...
    if (a) {
        ...
        [...AppState.activityLog].reverse().forEach(function(b) {
            ...
            c.className = "activity-item", c.innerHTML = `<div class="activity-dot"></div><div class="activity-content"><div class="activity-text">${b.text}</div><div class="activity-time">${formatTime(b.timestamp)}</div></div>`, a.appendChild(c)
        })
    }
}

function attachColumnListeners(a) {
    var b = a.dataset.id;
    a.querySelector(".add-card-btn").addEventListener("click", function() {
        openCardModal(null, b)
    }), a.querySelector(".edit-col").addEventListener("click", function() {
        openColumnModal(b)
    }), a.querySelector(".delete-col").addEventListener("click", function() {
        confirmDelete("Are you sure you want to delete this column and all its cards?", function() {
            var c = getCardsForColumn(b);
            c.forEach(function(a) {
                delete AppState.cards[a.id]
            }), AppState.columns = AppState.columns.filter(function(a) {
                return a.id !== b
            }), addActivity("Deleted column"), autoSave(), renderBoard()
        })
    })
}